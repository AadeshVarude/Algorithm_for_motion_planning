...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            x_center=Node((self.start.row+self.goal.row)/2 ,(self.start.col+self.goal.col)/2)
         
            # Compute rotation matrix from elipse to world frame - C
            mat=np.array([[(self.start.row - self.goal.row)/c_min],[(self.start.col-self.goal.col)/c_min],[0]])
            indentity=np.array([1,0,0]).reshape(1,3)
            M=np.dot(mat,indentity)
            U,S,V=np.linalg.svd(M,True,True)
            diag=np.dot(np.diag([1,1,np.linalg.det(U)*np.linalg.det(np.transpose(V))]),V)
            C=np.dot(U,diag)

            # Compute diagonal matrix - L
            r1=c_best/2
            r2=np.sqrt(c_best**2-c_min**2)/2
            r3=np.sqrt(c_best**2-c_min**2)/2
            L=np.diag([r1,r2,r3])
            ...
            x = r*math.cos(theta)
            y = r*math.sin(theta)
            x_ball=np.array([x,y,0]).reshape(3,1)
            # # Map ball sample to the ellipsoid - x_rand
            CLx=np.dot(C,np.dot(L,x_ball))
            x_rand=CLx+np.array([[x_center.row],[x_center.col],[0]])
