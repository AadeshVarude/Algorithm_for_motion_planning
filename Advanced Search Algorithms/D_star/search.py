...
class DStar:
    ...
    def process_state(self):
        ...
        if k_old < new_node.h:
            for neighbor in neighbors:
                if neighbor.h <= k_old and new_node.h > neighbor.h+ self.cost(new_node,neighbor):
                    new_node.parent=neighbor    
                    new_node.h = neighbor.h+ self.cost(new_node,neighbor)
        if k_old==new_node.h:
            for neighbor in neighbors:
                if neighbor.tag=="NEW" or (neighbor.parent==new_node and neighbor.h!=self.cost(new_node,neighbor)+new_node.h) or (neighbor.parent!=new_node and neighbor.h>self.cost(new_node,neighbor)+new_node.h):
                    neighbor.parent=new_node    
                    self.insert(neighbor,self.cost(new_node,neighbor)+new_node.h)
        else:
            for neighbor in neighbors:
                if neighbor.tag=="NEW" or (neighbor.parent==new_node and neighbor.h!=self.cost(new_node,neighbor)+new_node.h):
                    neighbor.parent=new_node    
                    self.insert(neighbor,self.cost(new_node,neighbor)+new_node.h) 
                else:
                    if neighbor.parent != new_node and neighbor.h > self.cost(new_node,neighbor)+new_node.h:
                        self.insert(new_node,new_node.h)
                    else:
                        if neighbor.parent != new_node and new_node.h > self.cost(new_node,neighbor)+neighbor.h and neighbor.tag=="CLOSED" and neighbor.h>k_old:
    ...
    def prepare_repair(self, node):
        ...
        #### TODO END ####
        # print("in pr",node.row)
        neighbors=self.get_neighbors(node)
        for neighbor in neighbors:
            if neighbor.is_dy_obs==True and neighbor.is_obs==False :
                if neighbor.tag == "CLOSED":
                    self.insert(neighbor, math.inf) # setting the closed node to initial conditions as the node needs to be reprocessed
                neighbor.is_obs=True # markin dynamic obstacle as obstacle
                # Modify the cost from this neighbor node to all this neighbor's neighbors
    ...
    def run(self):
        ...
        while(len(self.open)>0 or self.start.tag!="CLOSED"):
            k_min=self.process_state()
        ...
        new_node=self.start
        while(new_node!=self.goal):
            self.prepare_repair(new_node)
            self.repair_replan(new_node)
            self.get_backpointer_list(new_node)
